...
class SQL:
    ...
    def update_user(self, username: str, password: str, server: Servers, user_id: int):
        try:
            c = self.conn.cursor()
            c.execute("UPDATE users set username = ?, password = ?, server = ? where id = ?",(username, password, server.value, user_id),)
            self.conn.commit()
            c.close()
            return True
        except Exception as ex:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    context.user_data["telegram_id"] = update.message.from_user.id  # ذخیره آیدی تلگرام در user_data
...
async def username(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if is_farsi(update.message.text):
        await update.message.reply_text("لطفاً از حروف انگلیسی برای یوزرنیم استفاده کنید.")
        return USERNAME
    else:
        context.user_data["username"] = update.message.text
        await update.message.reply_text("یوزرنیم شما ذخیره شد. حالا لطفاً پسورد خود را وارد کنید:")
        return PASSWORD
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            SERVER: [
                MessageHandler(filters.Regex("^(Asia|Europe|America|TW/HK/MO)$"), server)
            ],
        },
